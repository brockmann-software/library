...
class My_Pdf_Table_Cell {
	...
	/**
	 * St style attributes with Zend_Pdf_Style or My_Pdf_Table_Cell_Style
	 *
	 * @param Zend_Pdf_Style or My_Pdf_Table_Cell_Style
	 */
	 public function setStyle($myStyle)
	 {
		 if ($myStyle instanceof My_Pdf_Table_Column_Style) {
			$this->_align = $myStyle->getTextAlign();
			$this->_vAlign = $myStyle->getVerticalAlign();
			$this->_bgColor = $myStyle->getBackgroundColor();
			$this->_textLineSpacing = $myStyle->getTextLineSpacing();
			$this->_border = $myStyle->getBorder();
			foreach ($myStyle->getPadding() as $key => $val) $this->setPadding($key, $val);
		 }
		 if ($myStyle instanceof Zend_Pdf_Style) {
			$this->setFont($myStyle->getFont(), $myStyle->getFontSize());
			$_color = $myStyle->getFillColor();
		 }
	 }
	...
	public function setHeight($value){
		if (is_numeric($value)) $this->_height=$value;
		elseif (is_array($value)) {
			if (!is_numeric($value[0])) throw new Zend_Pdf_Exception('First param of height has to be numeric');
			switch (strtoupper($value[1])) {
				case 'CM': $this->_height = ($value[0]*72/2.54);
					break;
				case 'IN': $this->_height = ($value[0]*72);
					break;
				case 'MM': $this->_height = ($value[0]*72/25.40);
					break;
				case 'PX': $this->_height = $value[0];
				default: throw new Zend_Pdf_Exception('This scale unit is not supported!');
			}
		}
	}
